...
public class PluginDBDumper extends AbstractVerticle
{
  private final static Logger LOGGER =
    Logger.getLogger(Logger.GLOBAL_LOGGER_NAME);

  MemoryDB memoryDumper;

  SqlClient sqlClient;

  CpuDB cpuDB;

  public PluginDBDumper(SqlClient sqlClient)
  {
    this.sqlClient =sqlClient;

    memoryDumper = new MemoryDB();
  ...
  public void start(Promise<Void> startPromise) throws Exception
  {
    ...
    System.out.println("Plugin DB Dumper Loaded");
    //dump the data in database
    vertx.eventBus().<String>localConsumer(Address.dumpDB,handler->{
      ...
      if(jsonObeject.getBoolean("status"))
      {
        ...
        if(metric.equals("memory"))
        {
          ...
          memoryDumper.saveMemory(sqlClient,memory_metric,timestamp);
        ...
        else if(metric.equals("cpu"))
        {
          ...
          cpuDB.saveCpu(sqlClient,cpu_metric,timestamp);
